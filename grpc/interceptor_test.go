...
import (
	"context"
	"github.com/jayleonc/geektime-go/webook/pkg/grpcx/interceptor/trace"
...
func (s *InterceptorTestSuite) TestClient() {
	...
	cc, err := grpc.Dial("localhost:8090",
		grpc.WithChainUnaryInterceptor(trace.NewOTELInterceptorBuilder("client_test", nil, nil).BuildUnaryClientInterceptor()),
...
func (s *InterceptorTestSuite) TestServer() {
	...
	server := grpc.NewServer(
		grpc.ChainUnaryInterceptor(NewLogInterceptor(t), trace.NewOTELInterceptorBuilder("server_test", nil, nil).BuildUnaryServerInterceptor()))
