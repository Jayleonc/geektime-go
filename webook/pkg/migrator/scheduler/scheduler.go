...
import (
	...
	"github.com/jayleonc/geektime-go/webook/pkg/migrator/validator"
	"gorm.io/gorm"
	"sync"
	"time"
)
...
func NewScheduler[T migrator.Entity](
	...
	pool *connpool.DoubleWritePool,
	producer producer.Producer) *Scheduler[T] {
	return &Scheduler[T]{
		l:       l,
		src:     src,
		dst:     dst,
...
// SrcOnly 只读写源表
func (s *Scheduler[T]) SrcOnly(c *gin.Context) (ginx.Response, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) SrcFirst(c *gin.Context) (ginx.Response, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) DstFirst(c *gin.Context) (ginx.Response, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) DstOnly(c *gin.Context) (ginx.Response, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) StopIncrementValidation(c *gin.Context) (ginx.Response, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	s.cancelIncr()
	return ginx.Response{
...
func (s *Scheduler[T]) StartIncrementValidation(c *gin.Context,
	req StartIncrRequest) (ginx.Response, error) {
	// 开启增量校验
	s.lock.Lock()
	defer s.lock.Unlock()
	// 取消上一次的
...
func (s *Scheduler[T]) StopFullValidation(c *gin.Context) (ginx.Response, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	s.cancelFull()
	return ginx.Response{
...
func (s *Scheduler[T]) StartFullValidation(c *gin.Context) (ginx.Response, error) {
	// 可以考虑去重的问题
	s.lock.Lock()
	defer s.lock.Unlock()
	// 取消上一次的
